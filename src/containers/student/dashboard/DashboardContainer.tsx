...
import { Calendar, Bell, CreditCard, MapPin, Star, Clock, Play, BookOpen, Users, FileText, Search, MessageSquare, Award } from 'lucide-react';
      ...
      <div className="bg-gradient-to-br from-emerald-400 via-emerald-500 to-teal-500 text-white px-6 pt-8 pb-32 rounded-b-[40px]">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-white text-opacity-90 text-sm">Selamat datang, Agus Siswanto</p>
          </div>
      ...
      <div className="relative px-6 -mt-20 pb-6">
        ...
        <div className="bg-white rounded-3xl shadow-xl p-8 mb-6">
          <div className="grid grid-cols-3 gap-6">
            <button className="flex flex-col items-center">
              ...
              <span className="text-sm font-medium text-gray-700 text-center">Cari Tutor</span>
            </button>
            <button className="flex flex-col items-center">
              <div className="bg-purple-50 p-5 rounded-2xl mb-3 hover:bg-purple-100 transition-all">
                <Calendar className="text-purple-500" size={28} strokeWidth={1.5} />
              </div>
              <span className="text-sm font-medium text-gray-700 text-center">Jadwal Kelas</span>
            </button>
            <button className="flex flex-col items-center">
              <div className="bg-teal-50 p-5 rounded-2xl mb-3 hover:bg-teal-100 transition-all">
                <BookOpen className="text-teal-500" size={28} strokeWidth={1.5} />
              </div>
              <span className="text-sm font-medium text-gray-700 text-center">Kelas Saya</span>
            </button>
          </div>
          ...
          <div className="grid grid-cols-3 gap-6 mt-6">
            <button className="flex flex-col items-center">
              <div className="bg-orange-50 p-5 rounded-2xl mb-3 hover:bg-orange-100 transition-all">
                <MessageSquare className="text-orange-500" size={28} strokeWidth={1.5} />
              </div>
              <span className="text-sm font-medium text-gray-700 text-center">Pesan</span>
            </button>
            <button className="flex flex-col items-center">
              <div className="bg-pink-50 p-5 rounded-2xl mb-3 hover:bg-pink-100 transition-all">
                <CreditCard className="text-pink-500" size={28} strokeWidth={1.5} />
              </div>
              <span className="text-sm font-medium text-gray-700 text-center">Pembayaran</span>
            </button>
            <button className="flex flex-col items-center">
              ...
              <span className="text-sm font-medium text-gray-700 text-center">Sertifikat</span>
            </button>
          </div>
        </div>
        ...
        <div className="mb-6">
          ...
          {announcements.map((item, idx) => (
            <div key={idx} className="bg-white rounded-2xl shadow-md p-5 mb-3 hover:shadow-lg transition-shadow">
              <div className="flex items-start">
                <div className={`p-3 rounded-xl mr-4 ${
                  item.category === 'Promo' ? 'bg-rose-50' : 'bg-blue-50'
                }`}>
                  <Bell className={`${
                    item.category === 'Promo' ? 'text-rose-500' : 'text-blue-500'
                  }`} size={20} />
                </div>
        ...
        {/* Categories */}
        <div className="mb-6">
          <h2 className="text-xl font-bold text-gray-800 mb-4">Kategori Pembelajaran</h2>
        ...
        <div>
          <div className="flex justify-between items-center mb-4">
            ...
            <button className="text-sm text-emerald-600 font-semibold">Lihat Semua →</button>
          </div>

          <div className="space-y-4">
            {filteredTeachers.map(teacher => (
              <div key={teacher.id} className="bg-white rounded-3xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow">
                <div className="p-6">
                    ...
                    <div className="flex-1">
                      <div className="flex items-start justify-between mb-1">
                        <div>
                          ...
                          <p className="text-emerald-600 text-sm font-semibold">{teacher.subject}</p>
                        </div>
                        {teacher.certified && (
                          <span className="bg-blue-50 text-blue-600 text-xs px-3 py-1.5 rounded-full font-semibold">✓ Verified</span>
                        )}
                      </div>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                    <div>
                      ...
                      <p className="text-base font-bold text-emerald-600">{teacher.price}</p>
                    </div>
