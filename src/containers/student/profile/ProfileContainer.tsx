...
export default function ProfileContainer() {
    ...
    const menuItems = [
        { icon: <Settings size={20} />, label: 'Pengaturan Akun', color: 'text-blue-600', bg: 'bg-blue-100' },
        { icon: <CreditCard size={20} />, label: 'Metode Pembayaran', color: 'text-purple-600', bg: 'bg-purple-100' },
        { icon: <Award size={20} />, label: 'Sertifikat Saya', color: 'text-yellow-600', bg: 'bg-yellow-100' },
        { icon: <BookOpen size={20} />, label: 'Riwayat Pembelajaran', color: 'text-emerald-600', bg: 'bg-emerald-100' },
        { icon: <Bell size={20} />, label: 'Notifikasi', color: 'text-orange-600', bg: 'bg-orange-100' },
        { icon: <Shield size={20} />, label: 'Privasi & Keamanan', color: 'text-red-600', bg: 'bg-red-100' },
        { icon: <HelpCircle size={20} />, label: 'Bantuan & Dukungan', color: 'text-indigo-600', bg: 'bg-indigo-100' },
    ];
            ...
            <div className="px-6 -mt-16">
                ...
                <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
                    <div className="flex items-center gap-4 mb-6">
                        <div
                            className="bg-gradient-to-br from-emerald-100 to-teal-100 rounded-full w-20 h-20 flex items-center justify-center text-4xl">
                            {userProfile.avatar}
                        </div>
                        <div className="flex-1">
                            <h2 className="text-xl font-bold text-gray-800 mb-1">{userProfile.name}</h2>
                            <p className="text-sm text-gray-600 mb-2">Member sejak {userProfile.joinDate}</p>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        {achievements.map(achievement => (
                            <div key={achievement.id}
                                ...
                                <h3 className="font-bold text-gray-800 text-xs mb-1">{achievement.title}</h3>
                    ...
                    {menuItems.map((item, index) => (
                        ...
                        >
                            <div className="flex items-center gap-3">
                                <div className={`${item.bg} w-10 h-10 rounded-full flex items-center justify-center`}>
                ...
                {/* Recent Activity */}
                <div className="mb-6">
                    <h2 className="text-lg font-bold text-gray-800 mb-3">Aktivitas Terbaru</h2>
                            ...
                            <div key={activity.id} className="bg-white rounded-xl shadow p-4">
                                <div className="flex items-start gap-3">
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
                                        ...
                                        {activity.type === 'class' && <BookOpen size={18} className="text-blue-600" />}
                                        {activity.type === 'achievement' && <Star size={18} className="text-yellow-600" />}
                                        {activity.type === 'payment' && <CreditCard size={18} className="text-emerald-600" />}
                                    </div>
                                    <div className="flex-1">
